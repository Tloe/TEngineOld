...
TE::Engine::EngineRoot::EngineRoot(IO::FileIO &fileIO, const std::string &initScrip)
    : m_fileIO(fileIO),
...
/*void TE::Engine::EngineRoot::JSONDeserialize(const Json::Value &jsonValue)
{
    ...
    {
        Event::EnvironmentUpdateEvent event(environmentVariable,
Core::Variant(jsonEnvironment[environmentVariable].asString())); m_eventManager.QueEvent(event);
    }
    ...
    for(const std::string & system : jsonSystems.getMemberNames())
    {
        auto systemItr = std::find_if(std::begin(m_systemFactories), std::end(m_systemFactories),
                                    [&system](Engine::SystemFactory* sysGenerator) { return
sysGenerator->GetSystemName() == system; });
...
I32 func(float f) { return static_cast<I32>(f); }
...
public:
  ...
  void Add(I32 iv) { std::cout << iv + m_i + m_j; }
...
TE::Event::EventManager &TE::Engine::EngineRoot::GetEventManager() { return m_eventManager; }

TE::Engine::SceneHandler &TE::Engine::EngineRoot::GetSceneHandler() { return m_sceneHandler; }

TE::IO::FileIO &TE::Engine::EngineRoot::GetFileIO() { return m_fileIO; }
