...
namespace TE::Engine {
  class EngineRoot : public Event::EventHandler,
                     public Lua::Extended
  ...
  public:
    ...
    template <typename SystemT> SystemT &GetSystem(const std::string &systemName) {
      auto findItr = std::find_if(
          std::begin(m_systems), std::end(m_systems),
    ...
    template <typename SystemT> SystemT &GetSystem(U32 systemId) {
      auto findItr =
          std::find_if(std::begin(m_systems), std::end(m_systems), [&systemId](SystemUPtr &sysPtr) {
            return sysPtr->GetSystemId() == systemId;
          });
  ...
  private:
    using SystemGeneratorVec = std::vector<SystemFactory *>;
