...
TE::Network::NetworkScene::NetworkScene(Event::EventManager &eventManager,
...
TE::Engine::SystemTask *TE::Network::NetworkScene::GetSystemTask() { return &m_networkTask; }

TE::Engine::SystemObjectSPtr
TE::Network::NetworkScene::CreateSystemObject(const std::string &objectType, I32 objectId) {
    if (objectType == "NetworkObject") {
        auto networkObject = std::make_shared<NetworkObject>(objectId, m_eventManager,
                                                             m_networkManager.GetMessagehandler());
