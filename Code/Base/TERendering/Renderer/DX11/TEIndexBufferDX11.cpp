    ...
    , m_apiContext(apiContext)
{
	...
	context.devcontext->Map(m_D3DindexBuffer, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &ms);   // map the buffer
	memcpy(ms.pData, indexBuffer.GetDataPtr(), m_indexBuffer.GetDataSize());           // copy the data
	context.devcontext->Unmap(m_D3DindexBuffer, NULL);                                     // unmap the buffer*/
}
