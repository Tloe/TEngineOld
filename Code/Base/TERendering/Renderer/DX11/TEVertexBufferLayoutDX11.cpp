...
TE::Render::APIVertexBufferLayout::APIVertexBufferLayout( Context::APIContext& apiContext, const Render::Mesh& mesh, const Render::Effect& effect )
    : m_usageCount(1)
    , m_apiContext(apiContext)
{
	...
	for (I32 i = 0; i < layoutElementCount; ++i)
	{
		...
		ied[i].SemanticName = "POSITION";//APIMapping::s_layoutSemantic[layoutElement.layoutSemantic].c_str();
		...
		ied[i].InputSlotClass = static_cast<D3D11_INPUT_CLASSIFICATION>(layoutElement.typeOfData);
		ied[i].InstanceDataStepRate = layoutElement.dataStepRate;
	}*/
	...
	apiContext.GetDevice()->CreateInputLayout(ied, numElements, blob->GetBufferPointer(), blob->GetBufferSize(), &m_D3D11layout);
