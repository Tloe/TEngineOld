...
namespace TE::CollisionDetection {
  ...
  public:
    void GenerateContacts(std::unordered_map<I32, Physics::PhysicsComponent> &physicsComponents,
                          std::unordered_map<I32, CollisionComponent> &collisionComponents,
                          std::vector<Intersection::ContactSet> &contactSets,
                          Real dt);

  private:
    void CreatePlaneVectorAndTree(std::unordered_map<I32, CollisionComponent> &collisionComponents,
                                  std::vector<CollisionComponent *> &planes,
                                  BVHNode &treeRoot);
    void CheckObjectsAgainstPlanes(std::vector<CollisionComponent *> &planes,
                                   std::unordered_map<I32, CollisionComponent> &collisionComponents,
                                   std::vector<Intersection::ContactSet> &contactSets);
    void FullCheckPotentialContacts(
