...
namespace TE::Math {
  ...
  template <typename Real> class Helpers {
  public:
    /**
     ...
     *	\return		void
     */
    // static  void covarianceMatrixOfTris(Matrix4D<Real> &cov, const Real *vertexData, unsigned
    // vDataCount);
     ...
     *	\return		void
     */
    static void extremePointsAlongDirection(Vector3D<Real> dir,
     ...
     *	\return		bool
     */
    static bool faceVisibleToPoint(const Vector3D<Real> &faceNormal,
                                   const Vector3D<Real> &facePt,
                                   const Vector3D<Real> &pt);
    ...
    cOfMassFromVertex(const Real *vertexData, unsigned vDataCount, Real totalMass);
     ...
     *	\return		void
     */
    static void clipConvexPolygonAgainstPlane(const Vector3D<Real> &normal,
                                              Real constant,
                                              I32 &numPts,
                                              Vector3D<Real> *pts);
     ...
     *	\return		void
     */
    static void coplanarRectangleRectangle(const Vector3D<Real> rectangle0[4],
                                           const Vector3D<Real> rectangle1[4],
                                           I32 &numPts,
                                           Vector3D<Real> *pts);
     ...
     *	\return		void
     */
    static void coplanarSegmentRectangle(const Vector3D<Real> segment[2],
                                         const Vector3D<Real> rectangle[4],
                                         I32 &numPts,
                                         Vector3D<Real> *pts);
     ...
     *	\return		void
     */
    static void segmentSegment(const Vector3D<Real> segment0[2],
                               const Vector3D<Real> segment1[2],
                               I32 &numPts,
                               Vector3D<Real> *pts);
     ...
     *	\return		void
     */
    static void colinearSegments(const Vector3D<Real> segment0[2],
                                 const Vector3D<Real> segment1[2],
                                 I32 &numPts,
                                 Vector3D<Real> *pts);
     ...
     *	\return		void
     */
    static void segmentThroughPlane(const Vector3D<Real> segment[2],
                                    const Vector3D<Real> &planeOrigin,
                                    const Vector3D<Real> &planeNormal,
                                    I32 &numPts,
                                    Vector3D<Real> *pts);

    static Matrix4D<Real> CreateTransformMatrix4D(const Vector3D<Real> &translation,
                                                  const Vector3D<Real> &scale,
                                                  const Quaternion<Real> &quat);

    static Matrix3D<Real> Matrix3DFromQuaternion(const Quaternion<Real> &quat);
  };

  template <typename Real>
  void TE::Math::Helpers<Real>::covarianceMatrixOfPts(TE::Math::Matrix4D<Real> &cov,
                                                      const Real *vertexData,
                                                      U64 vDataCount) {
    cov.SetIdentity();
  ...
  /*
  template<typename Real>
  void TE::Math::Helpers<Real>::covarianceMatrixOfTris( TE::Math::Matrix4D<Real> &cov, const Real
  *vertexData, unsigned vDataCount )
  {
          ...
          {
                  p = TE::Math::Vector3D<Real>(vertexData[k], vertexData[k + 1], vertexData[k +
  2]); q = TE::Math::Vector3D<Real>(vertexData[k + 3], vertexData[k + 4], vertexData[k + 5]); r =
  TE::Math::Vector3D<Real>(vertexData[k + 6], vertexData[k + 7], vertexData[k + 8]); A_k = ((q -
  p).Cross(r - p)).Length() / 2; A_h += A_k;
          ...
          for(unsigned i = 0; i < 3; ++i)
                  for(unsigned j = 0; j < 3; ++j)
                  {
                          ...
                          for (unsigned k = 0; k < vDataCount - 8; k += 9)
                          {
                                  p = TE::Math::Vector3D<Real>(vertexData[k], vertexData[k + 1],
  vertexData[k + 2]); q = TE::Math::Vector3D<Real>(vertexData[k + 3], vertexData[k + 4],
  vertexData[k + 5]); r = TE::Math::Vector3D<Real>(vertexData[k + 6], vertexData[k + 7],
  vertexData[k + 8]); A_k = ((q - p).Cross(r - p)).Length() / 2; M_k = (p + q + r) / 3; step = (
  A_k / 12 )*( (9 * M_k[i] * M_k[j]) + ( p[i] * p[j] ) + ( q[i] * q[j] ) + ( r[i] * r[j] ) );
                          }
  ...
  */
  ...
  template <typename Real>
  void TE::Math::Helpers<Real>::extremePointsAlongDirection(TE::Math::Vector3D<Real> dir,
                                                            const Real *vertexData,
                                                            unsigned vDataCount,
                                                            I32 &imin,
                                                            I32 &imax) {
  ...
  TE::Math::Helpers<Real>::symSchur2(TE::Math::Matrix4D<Real> &a, I32 p, I32 q, Real &c, Real &s) {
  ...
  template <typename Real>
  bool TE::Math::Helpers<Real>::faceVisibleToPoint(const TE::Math::Vector3D<Real> &faceNormal,
                                                   const TE::Math::Vector3D<Real> &facePt,
                                                   const TE::Math::Vector3D<Real> &pt) {
  ...
  template <typename Real>
  TE::Math::Vector3D<Real> TE::Math::Helpers<Real>::cOfMassFromVertex(const Real *vertexData,
                                                                      unsigned vDataCount,
                                                                      Real totalMass) {
  ...
  template<typename Real>
  TE::Math::Matrix4D<Real> TE::Math::Helpers<Real>::calcInertiaTensor( const Real *vertexData,
  unsigned vDataCount )
  {
  ...
  template <typename Real>
  TE::Math::Matrix3D<Real> TE::Math::Helpers<Real>::inertiaTensorOfSolidCuboid(Real mass,
                                                                               Real hight,
                                                                               Real width,
                                                                               Real depth) {
  ...
  template <typename Real> TE::Math::Matrix3D<Real>
  TE::Math::Helpers<Real>::inertiaTensorOfSolidSphere(Real mass, Real radius) {
    TE::Math::Matrix3D<Real> retMat;
    retMat.SetIdentity();
  ...
  template <typename Real> void
  TE::Math::Helpers<Real>::clipConvexPolygonAgainstPlane(const TE::Math::Vector3D<Real> &normal,
                                                         Real constant,
                                                         I32 &numPts,
                                                         TE::Math::Vector3D<Real> *pts) {
    ...
    for (i = 0; i < numPts; ++i) {
      ...
      test[i] = Dot(normal, pts[i]) - constant +
                Math::Base<Real>::Abs(constant) * TE::Math::Base<Real>::EPSILON;
  ...
  template <typename Real> void
  TE::Math::Helpers<Real>::coplanarRectangleRectangle(const TE::Math::Vector3D<Real> rectangle0[4],
                                                      const TE::Math::Vector3D<Real> rectangle1[4],
                                                      I32 &numPts,
                                                      TE::Math::Vector3D<Real> *pts) {
  ...
  template <typename Real> void
  TE::Math::Helpers<Real>::coplanarSegmentRectangle(const TE::Math::Vector3D<Real> segment[2],
                                                    const TE::Math::Vector3D<Real> rectangle[4],
                                                    I32 &numPts,
                                                    TE::Math::Vector3D<Real> *pts) {
  ...
  template <typename Real>
  void TE::Math::Helpers<Real>::segmentSegment(const TE::Math::Vector3D<Real> segment0[2],
                                               const TE::Math::Vector3D<Real> segment1[2],
                                               I32 &numPts,
                                               TE::Math::Vector3D<Real> *pts) {
    ...
    if (sqrLenN < TE::Math::Base<Real>::EPSILON * sqrLen0 * sqrLen1)
      ...
      TE::Math::Helpers<Real>::segmentThroughPlane(
          segment1, segment0[0], Cross(normal, segment0[1] - segment0[0]), numPts, pts);
  }

  template <typename Real>
  void TE::Math::Helpers<Real>::colinearSegments(const TE::Math::Vector3D<Real> segment0[2],
                                                 const TE::Math::Vector3D<Real> segment1[2],
                                                 I32 &numPts,
                                                 TE::Math::Vector3D<Real> *pts) {
  ...
  template <typename Real>
  void TE::Math::Helpers<Real>::segmentThroughPlane(const TE::Math::Vector3D<Real> segment[2],
                                                    const TE::Math::Vector3D<Real> &planeOrigin,
                                                    const TE::Math::Vector3D<Real> &planeNormal,
                                                    I32 &numPts,
                                                    TE::Math::Vector3D<Real> *pts) {
    numPts     = 1;
  ...
  template <typename Real> TE::Math::Matrix4D<Real>
  TE::Math::Helpers<Real>::CreateTransformMatrix4D(const TE::Math::Vector3D<Real> &translation,
                                                   const TE::Math::Vector3D<Real> &scale,
                                                   const TE::Math::Quaternion<Real> &quat) {
    // Create rotation transformation from quaternion
    TE::Math::Matrix4D<Real> returnMat(1 - (2 * quat.y * quat.y) - (2 * quat.z * quat.z),
  ...
  template <typename Real> TE::Math::Matrix3D<Real>
  TE::Math::Helpers<Real>::Matrix3DFromQuaternion(const TE::Math::Quaternion<Real> &quat) {
    // Create rotation transformation from quaternion
    return TE::Math::Matrix3D<Real>(1 - (2 * quat.y * quat.y) - (2 * quat.z * quat.z),
                                    (2 * quat.x * quat.y) - (2 * quat.z * quat.w),
                                    (2 * quat.x * quat.z) + (2 * quat.y * quat.w),
