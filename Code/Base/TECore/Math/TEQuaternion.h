...
namespace TE::Math {
  template <typename Real> class Matrix4D;
  template <typename Real> class Matrix3D;
  ...
  public:
    ...
    Vector3D<Real> &GetVector3D() { return (*reinterpret_cast<Vector3D<Real> *>(this)); }
    ...
    Quaternion<Real> &operator/=(const Quaternion<Real> &q) { return (*this *= Inverse(q)); }

    Quaternion<Real> &operator/=(const Vector3D<Real> &v) { return (*this *= -v / SquaredMag(v)); }
    Quaternion &operator/=(Real s) {
    ...
    Vector3D<Real> GetDirectionX() const {
      return (
          Vector3D<Real>(Math::Base<Real>::REAL_ONE - Math::Base<Real>::REAL_TWO * (y * y + z * z),
                         Math::Base<Real>::REAL_TWO * (x * y + w * z),
                         Math::Base<Real>::REAL_TWO * (x * z - w * y)));
    }

    Vector3D<Real> GetDirectionY() const {
      return (
          Vector3D<Real>(Math::Base<Real>::REAL_TWO * (x * y - w * z),
                         Math::Base<Real>::REAL_ONE - Math::Base<Real>::REAL_TWO * (x * x + z * z),
                         Math::Base<Real>::REAL_TWO * (y * z + w * x)));
    }

    Vector3D<Real> GetDirectionZ() const {
      return (Vector3D<Real>(Math::Base<Real>::REAL_TWO * (x * z + w * y),
                             Math::Base<Real>::REAL_TWO * (y * z - w * x),
                             Math::Base<Real>::REAL_ONE -
                                 Math::Base<Real>::REAL_TWO * (x * x + y * y)));
    }
    ...
    Matrix3D<Real> GetRotationMatrix() const {
      return Matrix3D<Real>(
          Math::Base<Real>::REAL_ONE - Math::Base<Real>::REAL_TWO * (y * y + z * z),
          Math::Base<Real>::REAL_TWO * (x * y - w * z),
          Math::Base<Real>::REAL_TWO * (x * z + w * y),
          Math::Base<Real>::REAL_TWO * (x * y + w * z),
          Math::Base<Real>::REAL_ONE - Math::Base<Real>::REAL_TWO * (x * x + z * z),
          ...
          Math::Base<Real>::REAL_TWO * (x * z - w * y),
          Math::Base<Real>::REAL_TWO * (y * z + w * x),
          Math::Base<Real>::REAL_ONE - Math::Base<Real>::REAL_TWO * (x * x + y * y));
    }
  ...
  template <typename Real> inline Quaternion<Real> operator-(const Quaternion<Real> &q) {
  ...
  template <typename Real> inline Quaternion<Real> operator+(const Quaternion<Real> &q, Real s) {
    return (Quaternion<Real>(q.x, q.y, q.z, q.w + s));
  }

  template <typename Real> inline Quaternion<Real> operator+(Real s, const Quaternion<Real> &q) {
  ...
  template <typename Real> inline bool operator==(const Quaternion<Real> &q, Real s) {
    return ((q.w == s) && (q.x == Math::Base<Real>::REAL_ZERO) &&
            (q.y == Math::Base<Real>::REAL_ZERO) && (q.z == Math::Base<Real>::REAL_ZERO));
  }

  template <typename Real> inline bool operator==(Real s, const Quaternion<Real> &q) {
    return ((q.w == s) && (q.x == Math::Base<Real>::REAL_ZERO) &&
            (q.y == Math::Base<Real>::REAL_ZERO) && (q.z == Math::Base<Real>::REAL_ZERO));
  }
  ...
  template <typename Real> inline Real Dot(const Quaternion<Real> &q1, const Quaternion<Real> &q2) {
    return (q1.x * q2.x + q1.y * q2.y + q1.z * q2.z + q1.w * q2.w);
  }

  template <typename Real> inline Real Magnitude(const Quaternion<Real> &q) {
    return (Base<Real>::Sqrt(q.x * q.x + q.y * q.y + q.z * q.z + q.w * q.w));
  }

  template <typename Real> inline Real SquaredMag(const Quaternion<Real> &q) {
    return (q.x * q.x + q.y * q.y + q.z * q.z + q.w * q.w);
  }

  template <typename Real> inline Quaternion<Real> Conjugate(const Quaternion<Real> &q) {
    return Quaternion<Real>(-q.x, -q.y, -q.z, q.w);
  }

  template <typename Real> inline Quaternion<Real> Inverse(const Quaternion<Real> &q) {
  ...
  Quaternion<Real> operator*(const Quaternion<Real> &q, const Vector3D<Real> &v) {
    return (Quaternion<Real>(q.w * v.x + q.y * v.z - q.z * v.y, q.w * v.y - q.x * v.z + q.z * v.x,
  ...
  Vector3D<Real> Rotate(const Vector3D<Real> &v, const Quaternion<Real> &q) {
    ...
    return (Vector3D<Real>(v.x * (Math::Base<Real>::REAL_ONE - Math::Base<Real>::REAL_TWO *
    (y2 + z2)) + v.y * Math::Base<Real>::REAL_TWO * (xy - wz) + v.z *
    Math::Base<Real>::REAL_TWO * (xz + wy), v.x * Math::Base<Real>::REAL_TWO * (xy + wz) +
    v.y * (Math::Base<Real>::REAL_ONE - Math::Base<Real>::REAL_TWO * (x2 + z2)) + v.z *
    Math::Base<Real>::REAL_TWO * (yz - wx), v.x * Math::Base<Real>::REAL_TWO * (xz - wy) +
    v.y * Math::Base<Real>::REAL_TWO * (yz - wx) + v.z * (Math::Base<Real>::REAL_ONE -
    Math::Base<Real>::REAL_TWO * (x2 + y2))));*/
  ...
  template <>
  inline const TE::Math::Quaternion<F32> TE::Math::Quaternion<F32>::QUATERNION_IDENTITY =
      Quaternion<F32>{0.0f, 0.0f, 0.0f, 1.0};
  template <>
  inline const TE::Math::Quaternion<F64> TE::Math::Quaternion<F64>::QUATERNION_IDENTITY =
      Quaternion<F64>{0.0, 0.0, 0.0, 1.0};

  template <> inline const TE::Math::Quaternion<F32> TE::Math::Quaternion<F32>::QUATERNION_INVALID =
      Quaternion<F32>{0.0f, 0.0f, 0.0f, 0.0};
  template <> inline const TE::Math::Quaternion<F64> TE::Math::Quaternion<F64>::QUATERNION_INVALID =
