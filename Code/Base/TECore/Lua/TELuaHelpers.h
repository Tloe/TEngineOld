...
namespace TE::Lua {
  ...
  template <std::size_t N, std::size_t... Is> struct IndicesBuilder
      : IndicesBuilder<N - 1, N - 1, Is...> {};

  // The base case where we define the type tag
  template <std::size_t... Is> struct IndicesBuilder<0, Is...> { using Type = Indices<Is...>; };
