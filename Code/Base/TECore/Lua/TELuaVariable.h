...
namespace TE::Lua {
  ...
  private:
    ...
    template <typename T> void Store(T value) const;
  ...
  template <typename... Args> const Variable Variable::operator()(Args &&...args) {
  ...
  template <typename... Ts> Variable::operator std::tuple<Ts...>() const {
  ...
  template <typename T> void Variable::Store(T value) const {
    StackReseter stackReseter(m_state);
    TraverseRefs(m_state, m_keyPath);
    m_keyRef.Push();
    Push(m_state, value);
