...
namespace TE::Lua {
  ...
  template <typename T, typename... Ts> void Push(State &state, const T value, const Ts... values) {
    Push(value);
    Push(values...);
  }

  template <typename T> T ReadStack(Id<T> t, State &state, I32 index = -1) {
  ...
  std::tuple<Ts...> ReadStackN(State &state, I32 n, Indices<N...>) {
    ...
    saved = std::make_tuple(
        ReadStack<NoRef<Ts>>(Id<NoRef<Ts>>{}, state, -(n - static_cast<I32>(N)))...);
