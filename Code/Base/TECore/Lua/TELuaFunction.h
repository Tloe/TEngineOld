...
namespace TE::Lua {
  ...
  template <int N, typename Ret, typename... Args> class Function : public BaseFunction {
  ...
  template <int N, typename Ret, typename... Args>
  Function<N, Ret, Args...>::Function(State &state, const std::string &name, FunctionType function)
      : m_function(function),
        m_name(name),
        m_state(state) {
    lua_pushlightuserdata(StateToNative(m_state),
                          static_cast<void *>(static_cast<BaseFunction *>(this)));
    lua_pushcclosure(StateToNative(state), &LuaDispatcher, 1);
    lua_setglobal(StateToNative(state), m_name.c_str());
  }

  template <int N, typename Ret, typename... Args> Function<N, Ret, Args...>::~Function() {
    lua_pushnil(StateToNative(m_state));
    lua_setglobal(StateToNative(m_state), m_name.c_str());
  }

  template <int N, typename Ret, typename... Args> I32 Function<N, Ret, Args...>::Execute() {
