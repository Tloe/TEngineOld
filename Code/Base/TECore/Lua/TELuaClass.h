...
namespace TE::Lua {
  ...
  template <typename T, typename ConstructorT, typename... Members> class Class : public BaseClass {
  public:
  ...
  Class<T, ConstructorT, Members...>::Class(State &state,
  ...
  template <typename T, typename ConstructorT, typename... Members>
  template <typename Ret, typename... Args>
  void Class<T, ConstructorT, Members...>::RegisterMember(const std::string &name,
                                                          Ret (T::*function)(Args...)) {
    m_functions[name] = std::make_unique<ClassFunction<RetCount<Ret>::Count, T, Ret, Args...>>(
        m_state, name, m_metatableName, function);
  }

  template <typename T, typename ConstructorT, typename... Members>
  template <typename M, typename... Ms>
  void Class<T, ConstructorT, Members...>::RegisterMembers(const std::string &name,
                                                           M member,
                                                           Ms... members) {
