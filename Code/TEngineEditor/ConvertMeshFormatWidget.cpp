...
void ConvertMeshFormatWidget::Convert()
{
	...
	{
		I32 flags = 0;
		if(m_verticesCheck->isChecked()) flags ^= TE::IO::VERTEXBUFFER_POSITION;
		if(m_normalsCheck->isChecked()) flags ^= TE::IO::VERTEXBUFFER_NORMALS;
		if(m_textureCoordsCheck->isChecked()) flags ^= TE::IO::VERTEXBUFFER_TEXTURECOORDS;
