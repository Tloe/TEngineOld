...
void TE::IO::File::ReadData( std::vector<U8>& data, std::streamoff offset /*= -1*/, I32 size /*= 0*/ )
{
	if (size == 0)
	{
		...
		if (offset == -1) m_file->seekg(oldOffset, std::ios::beg);
	}

	if (offset >= 0) m_file->seekg(offset, std::ios::beg);
...
void TE::IO::File::ReadData( U8* data, std::streamoff offset /*= -1*/, I32 size /*= 0*/ )
{
	if (size == 0)
	{
		...
		if (offset == -1) m_file->seekg(oldOffset, std::ios::end);
	}
...
{
	if(offset >= 0) m_file->seekp(offset, std::ios::beg);
...
void TE::IO::File::RemoveFileData( const std::string& filePath,std::streamoff offset, I32 deleteSize /*= 0*/ )
{
	...
	if (deleteSize == 0) deleteSize = totalsize - static_cast<I32>(offset);
	...
	while (currentOffset != totalsize)
	{
		if (notDeleted)
		{
			if (bufferSize > offset - currentOffset)
			{
				...
				if(readSize) m_file->read(reinterpret_cast<char*>(&buffer[0]), readSize);
				notDeleted = false;
		...
		if(readSize) tempFile.write(reinterpret_cast<char*>(&buffer[0]), readSize);
	}
	tempFile.close();
...
void TE::IO::File::Open(const std::string& filePath, const std::fstream::ios_base::openmode openMode)
{
...
void TE::IO::File::SetReadOffset( std::streamoff offset )
{
	m_file->seekg(offset, std::ios::beg);
}

void TE::IO::File::ReadLine(std::string& line)
{
	getline(*m_file, line);
}

void TE::IO::File::SetWriteOffset( std::streamoff offset )
{
