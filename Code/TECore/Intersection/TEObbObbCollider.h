...
namespace TE
{
	...
	namespace Intersection { class intersectConfig; }
	...
	namespace Intersection
	{
		...
		{
		public:
			static bool Collide(const Obb& obb0, const Obb& obb1, Real dt, bool coarse, bool symetric,
		...
		private:
			...
			/**
			 *  \brief		Dynamic check, first time of contact(if any) is computed. No contact info
			 *	\Param		F32 tmax
			 *	\Param		Math::Vector3D<Real> velocity0
			 *	\Param		Math::Vector3D<Real> velocity1
			 *	\Param		Obb obbs[]
			 *	\return		bool
			 */	
			static bool DynamicCheck( const Obb& obb0, const Obb& obb1, F32 tmax, Math::Vector3D<Real> velocity0, Math::Vector3D<Real> velocity1);
			 ...
			 *	\return		bool
			 */	
			static bool DeriveContacts(const Obb& obb0, const Obb& obb1,F32 tmax, const Math::Vector3D<Real>& velocity0, const Math::Vector3D<Real>& velocity1, ContactSet* contacts = NULL);
			 ...
			 *	\return		bool
			 */	
			static bool IsSeparated(F32 min0, F32 max0, F32 min1, F32 max1, F32 speed, F32 tmax, F32& tlast);
			 ...
			 *	\return		bool
			 */	
			static bool FindintersectionOnAxis(const Obb& obb0, const Obb& obb1,const Math::Vector3D<Real>& axis,
				const Math::Vector3D<Real>& velocity, F32 tmax, F32& tfirst, F32& tlast, I32& side, intersectConfig& box0CfgFinal,
				intersectConfig& box1CfgFinal);
		
			/**
			 *  \brief		finds the contact set based on 2 intersection configurations from 2 Obb's
			 *	\Param		const Obb obbs[]
			 *	\Param		const Math::Vector3D<Real> & velocity0
			 *	\Param		const Math::Vector3D<Real> & velocity1
			 *	\Param		I32 side
			 *	\Param		const intersectConfig & box0Cfg
			 *	\Param		const intersectConfig & box1Cfg
			 *	\Param		F32 tfirst
			 *	\return		void
			 */	
			static void FindContactSet(const Obb& obb0, const Obb& obb1, const Math::Vector3D<Real> &velocity0, const Math::Vector3D<Real> &velocity1, I32 side, const intersectConfig& box0Cfg, const intersectConfig& box1Cfg, F32 tfirst, ContactSet* contacts = NULL);
		};
	}
}
