...
{
	ExpandBuffer(1);
	if (value) m_buffer[m_bitIndex / 8] |= static_cast<U8>(1 << (7 - m_bitIndex % 8));
	++m_bitIndex;
}
...
bool TE::Net::Packet::ReadF32(F32& value)
{
	...
	U32 uValue;
	if (!ReadU32(uValue)) return false;
...
bool TE::Net::Packet::ReadF64(F64& value)
{
	...
	U32 ints[2];
	if (!ReadU32(ints[0])) return false;
	if (!ReadU32(ints[1])) return false;
...
bool TE::Net::Packet::ReadCustomResolutionF32(F32& value, F32 min, F32 max, I32 bitResolution)
{
	...
	value = 0;
	U32 uValue;
	if (!ReadU32(uValue, bitResolution)) return false;
	if (min < 0 && max > 0)
	{
...
bool TE::Net::Packet::ReadString(std::string& value, I32 bits)
{
	...
	value = "";
	U32 size;
	if (!ReadDynamicU32(size, bits)) return false;
...
bool TE::Net::Packet::ReadBinaryPacket(TE::Net::Packet& value)
{
	...
	if (!ReadDynamicU32(valueMaxBitIndex)) return false;
	for (U32 copyBytes = 0; copyBytes < valueMaxBitIndex / 8; ++copyBytes)
	{
		U8 valueByte;
		if (!ReadU8(valueByte)) return false;
