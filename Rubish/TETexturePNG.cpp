...
TE::Render::TexturePNG::TexturePNG()
	:
	Texture("")
{
...
TE::Render::TexturePNG::TexturePNG( const std::string& filename )
	:
	Texture(filename),
	m_prepared(false)
{
...
void TE::Render::TexturePNG::Prepare()
{
	if (m_prepared) return;

	std::vector<U8>& dataRef = GetData();
	...
	assert(png_check_sig(&dataRef[0], SIGNATURE_LENGTH) && "PNG file expected");
	...
	PNGData datastruct;
	datastruct.data = static_cast<png_bytep>(&dataRef[0]);
	datastruct.size = static_cast<png_uint_32>(dataRef.size());
	...
	rowPtrs = new0 png_bytep[height];
	I32 dataSize = width * height * bitDepth * channels / 8;
	data = new0 char[dataSize];
	...
	dataRef.clear();
	dataRef.insert(dataRef.begin(), data, data+dataSize);

	//Setup texture info
	m_textureInfo.width = width;
	m_textureInfo.height = height;
	m_textureInfo.bitDepth = bitDepth;
	...
	case PNG_COLOR_TYPE_GRAY:
		m_textureInfo.colorType = TextureInfo::COLORTYPE_RGB;
		break;
	case PNG_COLOR_TYPE_RGBA:
		m_textureInfo.colorType = TextureInfo::COLORTYPE_RGBA;
		break;
	default:
		std::clog << "Unsuported color_type" << std::endl;
	}

	if(alphaAdded) m_textureInfo.colorType = TextureInfo::COLORTYPE_RGBA;

	m_prepared = true;
}

TE::Render::TextureInfo& TE::Render::TexturePNG::GetTextureInfo()
{
