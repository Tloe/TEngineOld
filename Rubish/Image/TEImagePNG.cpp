...
TE::Images::ImagePNG::ImagePNG( const std::string& filename )
	:
	Image(filename),
	m_prepared(false)
{
...
void TE::Images::ImagePNG::Prepare()
{
	...
	std::vector<uint8_t>& dataRef = GetData();
	...
	case PNG_COLOR_TYPE_GRAY:
		m_imageInfo.colorType = ImageInfo::COLORTYPE_RGB;
		break;
	case PNG_COLOR_TYPE_RGBA:
		m_imageInfo.colorType = ImageInfo::COLORTYPE_RGBA;
		break;
	default:
		std::clog << "Unsuported color_type" << std::endl;
	}

	if(alphaAdded) m_imageInfo.colorType = ImageInfo::COLORTYPE_RGBA;

	m_prepared = true;
}

TE::Images::ImageInfo& TE::Images::ImagePNG::GetImageInfo()
{
	return m_imageInfo;
}
